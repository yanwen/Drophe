...
class BaiduBCS {
	...
	//SDK 版本
	const API_VERSION = '2011-08-29-1.0.0.2';
	const ACL = 'acl';
	const BUCKET = 'bucket';
	const OBJECT = 'object';
	...
	static $ACL_EFFECTS = array (
			self::BCS_SDK_ACL_EFFECT_ALLOW, self::BCS_SDK_ACL_EFFECT_DENY );
	//ACL_TYPE:
	//公开读权限
	 ...
	 * @return ResponseCore
	 */
	public function list_bucket($opt = NULL) {
		//$opt array
	 ...
	 * @return ResponseCore
	 */
	public function create_bucket($bucket, $acl = NULL, $opt = NULL) {
		//构造 $opt array
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
	 ...
	 */
	public function delete_bucket($bucket, $opt = NULL) {
		//$opt array
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
	 ...
	 * @return boolean|ResponseCore
	 */
	public function set_bucket_acl($bucket, $acl, $opt = NULL) {
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
		if (! $opt) {
	 ...
	 * @return ResponseCore
	 */
	public function list_object($bucket, $opt = NULL) {
		//$opt array
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
	 ...
	 * @return ResponseCore
	 */
	public function list_object_by_dir($bucket, $dir = '/', $list_model = 2, $opt = NULL) {
		//$opt array
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
	 ...
	 * @return ResponseCore
	 */
	public function create_object($bucket, $object, $file, $opt = NULL) {
		//$opt array
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
	 ...
	 * @return ResponseCore
	 */
	public function create_object_by_content($bucket, $object, $content, $opt = NULL) {
		//$opt array
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
	 ...
	 * @return ResponseCore
	 */
	public function create_object_superfile($bucket, $object, $file, $opt = NULL) {
		...
		$sliceNum = ceil ( $fileSize / $sub_object_size );
		...
		for($i = 0; $i < $sliceNum; $i ++) {
			...
			$opt ['length'] = $i + 1 == $sliceNum ? $fileSize % $sub_object_size : $sub_object_size;
	 ...
	 * 
	 */
	public function upload_directory($bucket, $dir, $opt = NULL) {
		...
		$result = array (
				"success" => 0, "failed" => array (), "skipped" => 0 );
		$prefix = "";
		if (isset ( $opt ['prefix'] )) {
			$prefix = $opt ['prefix'];
		}
		...
		foreach ( $objects as $object => $file ) {
			...
			if (isset ( $opt [self::IMPORT_BCS_PRE_FILTER] )) {
				$bolRes = call_user_func ( $opt [self::IMPORT_BCS_PRE_FILTER], $bucket, $object, $file, &$tmp_opt );
				if ($bolRes !== true) {
			...
			if (isset ( $opt [self::IMPORT_BCS_POST_FILTER] )) {
				call_user_func ( $opt [self::IMPORT_BCS_POST_FILTER], $bucket, $object, $file, &$tmp_opt, $response );
			}
			$num ++;
		}
		//打印日志并返回结果数组
	 ...
	 * @return ResponseCore
	 */
	public function set_object_meta($bucket, $object, $meta, $opt = NULL) {
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
		if (! $opt) {
		...
		$source = array (
				self::BUCKET => $bucket, self::OBJECT => $object );
	 ...
	 * @return ResponseCore
	 */
	public function get_object_acl($bucket, $object, $opt = NULL) {
		//$opt array
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
	 ...
	 * @return ResponseCore
	 */
	public function set_object_acl($bucket, $object, $acl, $opt = NULL) {
		//$opt array
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
	 ...
	 * @return ResponseCore
	 */
	public function delete_object($bucket, $object, $opt = NULL) {
		//$opt array
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
	 ...
	 */
	public function is_object_exist($bucket, $object, $opt = NULL) {
		//$opt array
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
	 ...
	 * @return ResponseCore
	 */
	public function get_object_info($bucket, $object, $opt = NULL) {
		//$opt array
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
	 ...
	 * @param string $object (Required)
	 * @param array $opt (Optional)
	 * fileWriteTo   (Optional)直接将请求结果写入该文件
	 * @throws BCS_Exception
	 * @return ResponseCore
	 */
	public function get_object($bucket, $object, $opt = NULL) {
		//$opt array
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
	 ...
	 */
	public function generate_get_object_url($bucket, $object, $opt = NULL) {
		//$opt array
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
	 ...
	 * return false| string url
	 */
	public function generate_put_object_url($bucket, $object, $opt = NULL) {
		//$opt array
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
	 ...
	 * return false| string url
	 */
	public function generate_post_object_url($bucket, $object, $opt = NULL) {
		//$opt array
		if ($opt != NULL && ! is_array ( $opt )) {
			throw new BCS_Exception ( '$opt must be array, please check!', - 1 );
		}
	...
	public function log($log, $opt) {
          /*if (isset ( $opt [self::IMPORT_BCS_LOG_METHOD] )) {
			call_user_func ( $opt [self::IMPORT_BCS_LOG_METHOD], $log );
		} else {
			trigger_error ( $log );
}*/
	}
